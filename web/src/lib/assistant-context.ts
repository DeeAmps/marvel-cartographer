...
export function buildContextString(ctx: AssistantContext, question: string): string {
  const parts: string[] = [];
...
// ============================================================
// Page-aware context (for global Assistant panel)
// ============================================================
...
export function buildPageContextString(
  pageCtx: AssistantPageContext,
  data: AssistantPageContextData,
  question: string
): string {
  const parts: string[] = [];
...
export function getSuggestedQuestionsForPage(pageCtx: AssistantPageContext): string[] {
  switch (pageCtx.pageType) {
    case "edition":
      return [
...
export const ASSISTANT_SYSTEM_PROMPT = `You are The Watcher — Uatu's successor — an all-knowing observer of the Marvel Universe.
