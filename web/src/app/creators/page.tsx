    ...
    <div>
      ...
      <div className="flex flex-wrap gap-1.5 mb-4">
        ...
        {allRoles.map((r) => (
          <a
            key={r}
            href={`/creators?role=${encodeURIComponent(r)}${era ? `&era=${era}` : ""}`}
            className="px-2.5 py-1 rounded text-xs font-bold transition-all"
            style={{
      ...
      <div className="flex flex-wrap gap-1.5 mb-6 items-center">
        ...
        {eraGroups.map((g) => {
          ...
          return (
            <a
              key={eraValue}
              href={`/creators?${role ? `role=${encodeURIComponent(role)}&` : ""}era=${eraValue}`}
        ...
        {era && (
          <a
            href={`/creators${role ? `?role=${encodeURIComponent(role)}` : ""}`}
