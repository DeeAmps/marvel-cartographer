...
import FaithfulnessBreakdown from "@/components/mcu/FaithfulnessBreakdown";
import ComicSourceList from "@/components/mcu/ComicSourceList";
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const content = await getMCUContentBySlug(slug);
  if (!content) return { title: "Not Found" };
  return {
    title: `${content.title} — MCU Cross-Reference`,
    description: `Comic book sources for ${content.title}. Faithfulness score: ${content.faithfulness_score}%.`,
  };
}

export default async function MCUDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const content = await getMCUContentBySlug(slug);
  if (!content) notFound();

  const mappings = await getMCUMappingsForContent(content.id);
    ...
    <div className="max-w-3xl mx-auto">
      ...
      {mappings.length > 0 && (
        ...
        >
          ...
          <Link
            href={`/edition/${mappings[0].edition_slug}`}
