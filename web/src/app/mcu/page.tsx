...
import MCUContentCard from "@/components/mcu/MCUContentCard";
import { Film, Tv, Sparkles } from "lucide-react";

export const revalidate = 3600;

export const metadata = {
  title: "MCU Cross-Reference — The Marvel Cartographer",
  description: "Discover which comics inspired every MCU movie, series, and special. See faithfulness scores and read the original source material.",
};
    ...
    <div className="space-y-12">
      ...
      <section className="text-center py-8">
        ...
        <p className="text-sm max-w-lg mx-auto" style={{ color: "var(--text-secondary)" }}>
          Every MCU movie and series mapped to its comic book source material.
        ...
        .map(([phase, phaseMovies]) => (
          <section key={phase}>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
              {phaseMovies.map((m) => (
                <MCUContentCard key={m.slug} content={m} />
              ))}
            </div>
          </section>
        ))}
      ...
      {series.length > 0 && (
        <section>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
            {series.map((s) => (
              <MCUContentCard key={s.slug} content={s} />
            ))}
          </div>
        </section>
      )}
