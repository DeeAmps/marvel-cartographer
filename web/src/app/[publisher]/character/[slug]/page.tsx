...
async function buildCharacterReadingOrder(
  ...
  const connections = await getConnections(publisher);
...
export default async function CharacterDetailPage({
  ...
  const editions = await getEditionsByCharacter(slug, publisher);
  const readingOrder = await buildCharacterReadingOrder(editions, publisher);
        ...
        {character.teams.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-4">
            ...
            {character.teams.map((team) => (
              <Link
                key={team}
                href={`/${publisher}/characters?team=${encodeURIComponent(team)}`}
        ...
        >
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <Link
                href={`/${publisher}/handbook/${handbookEntry.slug}`}
        ...
        <section className="mb-8">
          ...
          <div className="space-y-2">
            {readingOrder.map((edition, idx) => (
              <Link
                key={edition.slug}
                href={`/${publisher}/edition/${edition.slug}`}
        ...
        <section className="mb-8">
          ...
          <div className="flex flex-wrap gap-2">
            {relatedCharacters.map((rc) => (
              <Link
                key={rc.slug}
                href={`/${publisher}/character/${rc.slug}`}
