...
export async function generateMetadata({
  ...
  const entry = await getHandbookEntryBySlug(slug, publisher);
...
export default async function HandbookDetailPage({
  ...
  const [entry, eras, editions, events, conflicts, allEntries] = await Promise.all([
    getHandbookEntryBySlug(slug, publisher),
    getEras(publisher),
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <nav className="flex items-center gap-1.5 text-xs mb-6" style={{ color: "var(--text-tertiary)" }}>
        <Link href={`/${publisher}/handbook`} className="hover:text-[var(--accent-red)] transition-colors">
          Handbook
        </Link>
        <span>/</span>
      ...
      {entry.entry_type === "character" && (
        <CharacterSection data={entry.data as CharacterHandbookData} handbookMap={handbookMap} publisher={publisher} />
      )}
        ...
        <section className="mb-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {relatedEditions.map((ed) => (
              <Link
                key={ed!.slug}
                href={`/${publisher}/edition/${ed!.slug}`}
        ...
        <section className="mb-10">
          ...
          <div className="space-y-2">
            {relatedConflicts.map((c) => (
              <Link
                key={c!.slug}
                href={`/${publisher}/conflicts#${c!.slug}`}
        ...
        <section className="mb-10">
          ...
          <div className="flex flex-wrap gap-2">
            {relatedHandbook.map((h) => (
              <Link
                key={h!.slug}
                href={`/${publisher}/handbook/${h!.slug}`}
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all hover:opacity-80"
                style={{
                  background: "var(--bg-tertiary)",
            ...
            {data.affiliations.map((aff) => {
              ...
              return (
                ...
                >
                  {team ? (
                    <Link
                      href={`/${publisher}/handbook/${team.slug}`}
