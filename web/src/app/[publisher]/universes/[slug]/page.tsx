...
export async function generateMetadata({
  ...
  const universe = await getUniverseBySlug(slug, publisher);
...
export default async function UniverseDetailPage({
  ...
  const [universe, editions, allUniverses] = await Promise.all([
    getUniverseBySlug(slug, publisher),
    getEditionsByUniverse(slug, publisher),
    getUniverses(publisher),
  ]);
        ...
        <section className="mb-8">
          ...
          <div className="flex flex-wrap gap-2">
            {otherUniverses.map((u) => (
              <Link
                key={u.slug}
                href={`/${publisher}/universes/${u.slug}`}
