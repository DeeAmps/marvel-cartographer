...
export async function generateMetadata({
  ...
  const event = await getEventWithPhases(slug, publisher);
    ...
    if (seg.type === "link") {
      return (
        <Link
          ...
          href={`/${publisher}/event/${seg.event.slug}`}
...
export default async function EventDetailPage({
  ...
  const [event, allEvents, readingOrder, allEditions] = await Promise.all([
    getEventWithPhases(slug, publisher),
    getEvents(publisher),
    getReadingOrderForEvent(slug, publisher),
  ...
  const eventEditions = await getEditionsForEvent(slug, publisher);
        ...
        {event.prerequisites && (
          ...
          >
            ...
            <p
              ...
              style={{ color: "var(--text-secondary)" }}
            >
              {linkifyEventText(event.prerequisites, allEvents, publisher)}
            </p>
          </section>
        )}
        {event.consequences && (
          ...
          >
            ...
            <p
              ...
              style={{ color: "var(--text-secondary)" }}
            >
              {linkifyEventText(event.consequences, allEvents, publisher)}
            </p>
          </section>
        )}
      </div>
          ...
          <div className="space-y-2">
            {eventEditions.map((item, idx) => (
              <Link
                ...
                href={`/${publisher}/edition/${item.edition.slug}`}
