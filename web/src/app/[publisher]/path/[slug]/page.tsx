...
export default async function ReadingPathPage({ params }: { params: Promise<{ publisher: string; slug: string }> }) {
  const { publisher: publisherParam, slug } = await params;
  ...
  const allPaths = await getReadingPaths(publisher);
          ...
          return (
            <div key={entry.position}>
              ...
              <div className="flex gap-2 sm:gap-4">
                ...
                {/* Edition card */}
                <Link href={`/${publisher}/edition/${entry.edition.slug}`} className="flex-1 group">
                  <div
