...
import AssistantVerdict from "@/components/assistant/AssistantVerdict";
...
export async function generateMetadata({ params }: { params: Promise<{ publisher: string; slug: string }> }) {
  ...
  const edition = await getEditionBySlug(slug, publisher);
...
export default async function EditionDetailPage({ params }: { params: Promise<{ publisher: string; slug: string }> }) {
  const { publisher: publisherParam, slug } = await params;
  ...
  const [{ outgoing, incoming }, graphData, allCharacters, storyArcs, retailers, editionIssueMap, handbookEntries, eras, prerequisites, mcuMappings] = await Promise.all([
    getConnectionsForEdition(slug, publisher),
    getMultiHopConnections(slug, 3, publisher),
    getCharacters(),
    getStoryArcsByEdition(slug, publisher),
    getRetailers(),
    getEditionIssueMap(publisher),
    getHandbookEntriesForEdition(slug, publisher),
    getEras(publisher),
    getPrerequisites(slug, publisher),
    getMCUMappingsForEdition(edition.id, publisher),
  ]);
  ...
  const allEditions = await getEditions(publisher);
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            itemListElement: [
              ...
              { "@type": "ListItem", position: 2, name: "Timeline", item: `/${publisher}/timeline` },
        ...
        <div className="flex-1">
          ...
          {edition.era_slug && (
            <div className="mb-4">
              ...
              <Link
                href={`/${publisher}/timeline#${edition.era_slug}`}
        ...
        >
          ...
          <div className="flex flex-wrap gap-2">
            {featuredCharacters.map((c) => (
              <Link
                key={c.slug}
                href={`/${publisher}/character/${c.slug}`}
          ...
          <div className="space-y-2">
            {mcuMappings.map((m) => {
              ...
              return (
                <Link
                  key={m.id}
                  href={`/${publisher}/media/${m.mcu_slug}`}
