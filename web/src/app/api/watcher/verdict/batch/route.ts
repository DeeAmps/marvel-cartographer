...
import { getEditions, getWatcherVerdict, getEditionBySlug, getConnectionsForEdition, getPrerequisites, saveWatcherVerdict } from "@/lib/data";
import type { WatcherVerdictData } from "@/lib/data";
...
export async function POST(request: NextRequest) {
  ...
  for (const edition of essentialEditions) {
    ...
    const cached = await getWatcherVerdict(edition.slug);
    if (cached) {
      skipped++;
      continue;
    }
...
Return ONLY valid JSON with this exact structure (no markdown, no explanation):
{
  ...
  "prerequisites": ["<edition title 1>", "<edition title 2>"]
      ...
      const verdict: WatcherVerdictData = JSON.parse(jsonMatch[0]);
      await saveWatcherVerdict(edition.slug, verdict, "claude-sonnet-4-5-20250929");
      generated++;
    } catch {
      failed++;
    }
