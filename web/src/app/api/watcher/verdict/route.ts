...
import { getEditionBySlug, getWatcherVerdict, saveWatcherVerdict, getConnectionsForEdition, getPrerequisites } from "@/lib/data";
...
export async function POST(request: NextRequest) {
  ...
  // Check cache first
  const cached = await getWatcherVerdict(editionSlug);
  if (cached) {
...
Return ONLY valid JSON with this exact structure (no markdown, no explanation):
{
  ...
  let verdict: WatcherVerdictData;
  try {
  ...
  // Cache the verdict
  await saveWatcherVerdict(editionSlug, verdict, "claude-sonnet-4-5-20250929");
