...
export default async function SearchPage({
  ...
  const [results, editionIssueMap, handbookResults] = await Promise.all([
    hasAnyFilter ? searchEditions(filters) : Promise.resolve([]),
    hasAnyFilter ? getEditionIssueMap() : Promise.resolve({}),
    query ? searchHandbook(query) : Promise.resolve([]),
  ]);
      ...
      {hasAnyFilter && (
        <div className="mt-8">
          ...
          {(() => {
            ...
            const showAllUrl = `/search?${showAllSearchParams.toString()}`;
