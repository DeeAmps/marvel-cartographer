...
export async function generateMetadata({ params }: { params: Promise<{ stone: string }> }) {
...
export default async function StoneDetailPage({ params }: { params: Promise<{ stone: string }> }) {
  const { stone } = await params;
  ...
  const editions = await getEditionsByTheme(theme);
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="flex items-center gap-4">
        <StoneIcon theme={theme} size={56} glow />
        <div>
          <h1
